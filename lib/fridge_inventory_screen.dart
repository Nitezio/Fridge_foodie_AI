...
class _FridgeInventoryScreenState extends State<FridgeInventoryScreen> {
  ...
  List<FridgeItem> get _filteredFridgeItems {
    if (_selectedCategoryFilter == null || _selectedCategoryFilter!.isEmpty) {
      ...
      return _fridgeItems.where((item) => item.category == _selectedCategoryFilter).toList();
    }
  }
  ...
  Future<void> _loadFridgeItems() async {
    ...
    setState(() {
      _fridgeItems = itemsJson.map((jsonString) => FridgeItem.fromJson(jsonDecode(jsonString))).toList();
    });
  }

  Future<void> _saveFridgeItems() async {
    final prefs = await SharedPreferences.getInstance();
    final itemsJson = _fridgeItems.map((item) => jsonEncode(item.toJson())).toList(); // Encode to JSON string
        ...
        children: [
          Padding(
            ...
            child: DropdownButtonFormField<String>(
              ...
              items: ['All', ..._categoryOptions.where((option) => option.isNotEmpty).toList()].map((String category) {
                  ...
                  child: ListTile(
                    ...
                    subtitle: Text(
                      'Expires: ${item.expiryDate.toLocal().toString().split(' ')[0]}' +
                          ...
                          (item.category != null && item.category!.isNotEmpty ? ', Category: ${item.category}' : ''),
                    ),
                    trailing: Row(
  ...
  Future<void> _showEditItemDialog(BuildContext context, int index, FridgeItem item) async {
